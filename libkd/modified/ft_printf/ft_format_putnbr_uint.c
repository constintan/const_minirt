...
static char	*kd_itoa_leadingzeros_uint(unsigned int n, int r, char *str)
{
	int		i;
	int		len;
...
static char	*kd_format_putnbr_uint_fill(t_format *list, char *s)
{
	int		i;
	int		j;
	int		k;
...
char	*kd_format_putnbr_uint(unsigned int n, t_format *list, char *s)
{
	...
	s = kd_itoa_leadingzeros_uint(n, list->precision, s);
	if (!s)
		return (s);
	list->maxlen = kd_max(kd_strlen(s), list->precision);
	s = kd_format_putnbr_uint_fill(list, s);
	return (s);
}
